---
title: 'Allowlisting Traffic'
description: 'Allowlisting traffic to Checkly'
sidebarTitle: 'Allowlisting Traffic'
---
...
You can then [set the Cookie header](/detect/synthetic-monitoring/api-checks/configuration#headers) while editing your check.

> You can set the header at group-level using [API check defaults](/platform/groups#headers--query-parameters) to have it applied to every API check in the group.
...
```ts checkly.config.ts highlight={15}
...
![filter google analytics traffic on campaign source](/images/docs/images/monitoring/analytics.png)
...
![CloudFlare verified bot WAF rule](/images/docs/images/monitoring/cloudflare-waf.png)
