...
You can add [setup and teardown scripts](/detect/synthetic-monitoring/api-checks/setup-and-teardown) at the group level as well as the 
...
<Note>Secrets are fully supported starting with runtime version 2024.09 and later. For [Private Locations](/platform/private-locations/overview), secrets are available in agent version `3.3.4` and later, and for the [CLI](/cli/overview), in version `4.9.0` and later.</Note>
