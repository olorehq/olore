...
[Browser checks](/browser-checks/), [API checks](/api-checks/), [Heartbeat monitors](/heartbeat-monitors/) and [Multistep checks](/multistep-checks/) share many arguments, configuration-wise, 
...
> Multistep checks are only supported on runtime 2023.09 or later. See [Runtimes](/runtimes/) for more details.
...
Once you start having more than just a handful of checks, it makes sense to start looking into [groups](/groups/) to keep things tidy:
