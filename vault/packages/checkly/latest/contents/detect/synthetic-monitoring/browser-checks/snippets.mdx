...
<Warning>
**Snippets are not supported in CLI-managed checks.** If you manage your checks with the [Checkly CLI](/cli/overview), use standard `import`/`require` against local files in your repo. See [local dependencies](/cli/dependencies#local-dependencies) for details.
</Warning>
...
Do you use [reusable code snippets for a setup or teardown script](/detect/synthetic-monitoring/api-checks/setup-and-teardown)?
...
![github login snippet example](/images/docs/images/browser-checks/github_login_snippet_example.png)
...
You should store these in [your environment variables](/platform/variables).
