...
* **Tags:** let you relate your checks together. They also determine which checks are shown on your [dashboards](/communicate/dashboards/overview)
...
To provide actionable and easy-to-read [check run results](/detect/synthetic-monitoring/multistep-checks/overview#multistep-check-results), we recommend using the `test.step()` structure when writing the Playwright script for you Multistep check.
