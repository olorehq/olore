---
title: Control and Configure your Monitoring Infrastructure with AI
...
The agent will then run `npx checkly init`, which generates a `checkly.config.ts` file and a `__checks__` directory. This base configuration file defines your project meta information, default check frequencies, and monitoring locations.
...
```ts base-url.check.ts highlight={16}
import { Frequency, UrlAssertionBuilder, UrlMonitor } from "checkly/constructs";
import { teamEmailAlert } from "./alert-channels";
...
```ts playwright.config.ts highlight={10-14}
...
```md generate-playwright-test.md
## Instructions
...
```typescript search-liquid.spec.ts
...
```typescript search-liquid.spec.ts highlight={4}
import { expect, test } from "@playwright/test";
