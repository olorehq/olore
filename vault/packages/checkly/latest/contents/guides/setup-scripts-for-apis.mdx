---
title: How to Use Setup and Teardown Scripts for Better API Monitoring
description: Setup and teardown scripts are fundamental tools to tailor API checks to your target endpoints. This guide presents real-world examples to help you master these tools.
tags: API Monitoring
sidebarTitle: Setup and Teardown Scripts
---

## The importance of self-contained checks

Checkly's [API Monitoring](/learn/monitoring/api-monitoring) checks run independently and on different schedules, possibly even following different retry logic when failures occur. It is therefore important not to create dependencies between them, which could introduce false failures and flakiness. Checkly prevents this antipattern by having each check run fully isolated in its own sandbox.
...
![checkly API check http request config](/images/guides/images/guides-checkly-setup-delete.png "The basic config for our API check's HTTP request")
...
![checkly API check http request](/images/guides/images/guides-checkly-setup-create.png "Our API check's HTTP request")
