...
2. Create a new project - developers can either grab an existing repository with the team's checks, or use `npx checkly init` to make a new project.
...
<Frame><img src="/images/guides/images/guide-fixtures01.png" alt="the Checkly Web UI with settings for a group, displaying variables" title="Setting variables that will be available to every check in this group" /></Frame>
...
<Frame><img src="/images/guides/images/guide-fixtures02.png" alt="the Checkly Web UI with settings for a group" title="Running a group from the Web UI's 'Schedule Group' option" /></Frame>
...
To open a modal and log in, we might use some lines like the following:

```ts RecentOrders.test.ts
...
```ts RecentOrdersWithFixture.test.ts
