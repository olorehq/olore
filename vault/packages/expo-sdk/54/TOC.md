...
│   ├── app.mdx         # app.json / app.config.js properties
│   ├── babel.mdx       # Babel configuration
│   ├── metro.mdx       # Metro bundler configuration
│   └── package-json.mdx# package.json fields
└── sdk/                # SDK API reference (106 files)
    ├── (sensors)       # accelerometer, barometer, devicemotion, gyroscope, light-sensor, magnetometer, pedometer, sensors
    ├── (media)         # audio, audio-av, av, camera, image, imagemanipulator, imagepicker, live-photo, video, video-av, video-thumbnails
    ├── (storage)       # asset, async-storage, blob, filesystem, filesystem-legacy, media-library, securestore, sqlite
    ├── (UI)            # blur-view, checkbox, date-time-picker, flash-list, gesture-handler, gl-view, glass-effect, image, linear-gradient, masked-view, mesh-gradient, picker, reanimated, safe-area-context, screen-orientation, screens, segmented-control, skia, slider, status-bar, svg, symbols, ui, view-pager
    ├── (device)        # application, battery, brightness, cellular, constants, device, network, netinfo, system-ui
    ├── (navigation)    # linking, router, router-native-tabs, router-ui
    ...
    └── (other)         # app-integrity, build-properties, calendar, captureRef, clipboard, contacts, crypto, dev-client, document-picker, expo, fingerprint, font, haptics, intent-launcher, keep-awake, keyboard-controller, location, mail-composer, manifests, maps, map-view, print, screen-capture, server, sharing, sms, speech, splash-screen, storereview, stripe, updates, webbrowser, webview
```
...
| File | Description |
|------|-------------|
| `sdk/audio.mdx` | Audio recording and playback (new API) |
| `sdk/audio-av.mdx` | Audio via AV module |
| `sdk/av.mdx` | Unified AV API (legacy) |
...
| `sdk/video-av.mdx` | Video via AV module |
...
| `sdk/ui.mdx` | expo-ui components |
| `sdk/view-pager.mdx` | ViewPager component |

### sdk/ — Device Info (9 files)
